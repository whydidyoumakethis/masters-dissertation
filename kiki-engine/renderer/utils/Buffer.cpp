...
namespace rutils {
    ...
    Buffer createBuffer(Allocator const& aAllocator, VkDeviceSize aSize, VkBufferUsageFlags aBufferUsage, VmaAllocationCreateFlags aMemoryFlags, VmaMemoryUsage aMemoryUsage) {
        ...
        if (auto const res = vmaCreateBuffer(aAllocator.allocator, &bufferInfo, &allocInfo,&buffer, &allocation, nullptr); VK_SUCCESS != res) {
            throw Kiki::FatalError( "Unable to allocate buffer.\n"
                "vmaCreateBuffer() returned {}", toString(res)
            );
        }

...
namespace rutils {
    Pipelines createAllPipelines(VulkanWindow const& window, PipelineLayouts const& pipelineLayouts) {
        ...
        pipelines.debug = createDebugPipeline(window, pipelineLayouts.debugPipelineLayout.handle);
    ...
    PipelineLayout createDebugPipelineLayout(VulkanWindow const& window, VkDescriptorSetLayout debugLayout) {
        ...
        pushRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
        pushRange.offset = 0;
        pushRange.size = sizeof(DebugData);
    ...
    PipelineLayout createChromaticAberrationPipelineLayout(VulkanWindow const& window, VkDescriptorSetLayout chromaticAberrationLayout) {
        VkDescriptorSetLayout layouts[] = {
            // Order must match the set = N in the shaders
            chromaticAberrationLayout // set 0
        };

        VkPushConstantRange pushRange{};
        pushRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
        pushRange.offset = 0;
        pushRange.size = sizeof(ChromaticAberrationSettings);
    ...
    PipelineLayout createCustomPostprocessPipelineLayout(VulkanWindow const& window, VkDescriptorSetLayout customPostprocessLayout) {
        ...
        pushRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
        pushRange.offset = 0;
        pushRange.size = sizeof(CustomPostprocessSettings);
    ...
    PipelineLayout createBloomPipelineLayout(VulkanWindow const& window, VkDescriptorSetLayout bloomLayout) {
        ...
        pushRange.stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT;
        pushRange.offset = 0;
        pushRange.size = sizeof(BloomData);
    ...
    Pipeline createShadowMapPipeline(VulkanWindow const& aWindow, VkPipelineLayout aPipelineLayout) {
        ...
        rasterInfo.depthBiasConstantFactor = 1.f;
        rasterInfo.depthBiasSlopeFactor = 2.f;
        rasterInfo.lineWidth = 1.f; // required
    ...
    Pipeline createChromaticAberrationPipeline(VulkanWindow const& aWindow, VkPipelineLayout aPipelineLayout) {
        ...
        // we only use the vertex and fragment shaders here
        auto const vShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.deferred_lighting_v.string().c_str());
        auto const fShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.chromatic_aberration_f.string().c_str());
    ...
    Pipeline createDebugPipeline(VulkanWindow const& aWindow, VkPipelineLayout aPipelineLayout) {
        ...
        // we only use the vertex and fragment shaders here
        auto const vShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.deferred_lighting_v.string().c_str());
        auto const fShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.debug_f.string().c_str());
    ...
    Pipeline createCustomPostprocessPipeline(VulkanWindow const& aWindow, VkPipelineLayout aPipelineLayout) {
        auto const vShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.deferred_lighting_v.string().c_str());
        auto const fShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.custom_postprocess_f.string().c_str());
    ...
    Pipeline createBloomDownsamplePipeline(VulkanWindow const& aWindow, VkPipelineLayout aPipelineLayout) {
        ...
        // we only use the vertex and fragment shaders here
        auto const vShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.deferred_lighting_v.string().c_str());
        auto const fShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.bloom_downsample_f.string().c_str());
    ...
    Pipeline createBloomUpsamplePipeline(VulkanWindow const& aWindow, VkPipelineLayout aPipelineLayout) {
        ...
        // we only use the vertex and fragment shaders here
        auto const vShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.deferred_lighting_v.string().c_str());
        auto const fShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.bloom_upsample_f.string().c_str());
        ...
        // define blend state
        // additive blending on upsampling only!
        VkPipelineColorBlendAttachmentState blendStates[1]{};
    ...
    Pipeline createDebugLinePipeline(VulkanWindow const& aWindow, VkPipelineLayout aPipelineLayout) {
        ...
        auto const fShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.debug_line_f.string().c_str());
        ...
        vertexInput.stride = sizeof(Kiki::DebugLineVertex);
        ...
        vertexAttributes[0].format = VK_FORMAT_R32G32B32_SFLOAT;
        vertexAttributes[0].offset = offsetof(Kiki::DebugLineVertex, position);

        vertexAttributes[1].binding = 0;
        vertexAttributes[1].location = 1;
        vertexAttributes[1].format = VK_FORMAT_R32G32B32A32_SFLOAT;
        vertexAttributes[1].offset = offsetof(Kiki::DebugLineVertex, colour);

        VkPipelineVertexInputStateCreateInfo inputInfo{};
        inputInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO;
        inputInfo.vertexBindingDescriptionCount = 1;
        inputInfo.pVertexBindingDescriptions = &vertexInput;
        ...
        VkPipeline pipe = VK_NULL_HANDLE;
        if (auto const res = vkCreateGraphicsPipelines(aWindow.device, VK_NULL_HANDLE, 1, &pipeInfo, nullptr, &pipe); VK_SUCCESS != res) {
            throw Kiki::FatalError("Unable to create debug line graphics pipeline\n"
                "vkCreateGraphicsPipelines() returned {}", rutils::toString(res)
            );
        }

...
namespace rutils {
    Pipelines createAllPipelines(VulkanWindow const& window, PipelineLayouts const& pipelineLayouts) {
        ...
        pipelines.ssao_hblur = createSSAOBlurPipeline(window, pipelineLayouts.ssaoBlurPipelineLayout.handle, 0);
        pipelines.ssao_blurred = createSSAOBlurPipeline(window, pipelineLayouts.ssaoBlurPipelineLayout.handle, 1);
    ...
    Pipeline createSSAOBlurPipeline(VulkanWindow const& aWindow, VkPipelineLayout aPipelineLayout, int direction) {
        // load shader code
        // we only use the vertex and fragment shaders here
        auto const vShader = rutils::loadShader(Kiki::RenderManager::get().shaderPaths.deferred_lighting_v.string().c_str());
        ...
        const char* path;
        if (direction == 0) {
            path = Kiki::RenderManager::get().shaderPaths.ssao_hblur_f.string().c_str();
        }
        else {
            path = Kiki::RenderManager::get().shaderPaths.ssao_vblur_f.string().c_str();
        }

        auto const fShader = rutils::loadShader(path);

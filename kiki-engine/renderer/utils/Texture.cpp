...
namespace rutils {
	...
	Texture::Texture(VmaAllocator aAllocator, VkImage aImage, VmaAllocation aAllocation, VkImageView aView) noexcept
		: image(aImage)
	...
	Texture loadImageTexture(char const* aPath, VulkanWindow const& aContext, VkCommandPool aCmdPool, Allocator const& aAllocator) {
        ...
        stbi_uc* data = stbi_load( aPath, &baseWidthi, &baseHeighti, &baseChannelsi, 4 /* want 4 c h a n n e l s = RGBA */ );

        if (!data) {
            throw Kiki::FatalError("{}: unable to load texture base image ({})", aPath, stbi_failure_reason());
        }
	...
	Texture createImageTexture(Allocator const& aAllocator, std::uint32_t aWidth, std::uint32_t aHeight, VkFormat aFormat, VulkanWindow const& window, VkImageUsageFlags aUsage) {
        ...
        return Texture(aAllocator.allocator, image, allocation, view);
	}
